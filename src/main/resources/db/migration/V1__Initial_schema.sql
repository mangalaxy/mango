...
CREATE TABLE IF NOT EXISTS company_benefits (
  ...
  CONSTRAINT company_benefit_fk FOREIGN KEY (company_id)
...
CREATE TABLE IF NOT EXISTS company_photos (
  ...
  CONSTRAINT company_photo_fk FOREIGN KEY (company_id)
    REFERENCES company (id)
);
...
CREATE TABLE IF NOT EXISTS company_links (
  ...
  CONSTRAINT company_link_fk FOREIGN KEY (company_id)
    REFERENCES company (id)
);
...
CREATE TABLE IF NOT EXISTS company_perks (
  company_id  BIGINT       NOT NULL,
  name        VARCHAR(60)  NOT NULL,
  CONSTRAINT company_perk_fk FOREIGN KEY (company_id)
    REFERENCES company (id)
);
...
CREATE TABLE IF NOT EXISTS employer (
  ...
  created_date TIMESTAMP WITHOUT TIME ZONE NOT NULL,
  last_update  TIMESTAMP WITHOUT TIME ZONE,
  CONSTRAINT company_employer_fk  FOREIGN KEY (company_id)
    REFERENCES company (id),
  CONSTRAINT location_employer_fk FOREIGN KEY (location_id)
...
CREATE TABLE IF NOT EXISTS job (
  ...
  created_date     TIMESTAMP WITHOUT TIME ZONE NOT NULL,
  last_update      TIMESTAMP WITHOUT TIME ZONE,
  CONSTRAINT location_job_fk FOREIGN KEY (location_id)
    REFERENCES location (id),
  CONSTRAINT employer_job_fk FOREIGN KEY (employer_id)
    REFERENCES employer (id),
  CONSTRAINT job_role_fk     FOREIGN KEY (job_role_id)
...
CREATE TABLE IF NOT EXISTS talent (
  ...
  created_date TIMESTAMP WITHOUT TIME ZONE NOT NULL,
  last_update  TIMESTAMP WITHOUT TIME ZONE,
  CONSTRAINT location_talent_fk FOREIGN KEY (location_id)
    REFERENCES location (id)
);
...
CREATE TABLE IF NOT EXISTS profile (
  ...
  CONSTRAINT talent_profile_fk   FOREIGN KEY (id)
    REFERENCES talent (id),
  CONSTRAINT location_profile_fk FOREIGN KEY (location_id)
    REFERENCES location (id)
);
...
-- Join table for bookmarked talents
